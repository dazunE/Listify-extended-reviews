...
<?php if ( have_comments() ) : ?>
    ...
    if ( is_singular( 'job_listing' ) && ( listify_has_integration( 'ratings' ) || listify_has_integration( 'wp-job-manager-reviews' ) ) && is_callable( 'Extended_comments_review', 'get_sort_options' ) ) : ?>

	<div class="comment-sorting-wrap">
		<h3 class="comment-sorting"><?php

            _e( 'Sort By', 'listify-child' ); ?></h3>
		<form action="#comments" method="get" class="comment-sorting-filter-form">
			<select name="sort-comments" class="comment-sorting-filter">
                ...
                foreach ( Extended_comments_review::get_sort_options() as $key => $label ) : ?>
		...
		<?php
			wp_list_comments(
				array(
					'style'      => 'ol',
					'short_ping' => true,
					'callback'   => 'extended_reviews_comments',
				)
			);
		?>
	</ol><!-- .commentlist -->
